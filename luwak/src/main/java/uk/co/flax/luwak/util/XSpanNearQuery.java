...
/**
 * Replacement class for {@link SpanNearQuery} for use by the highlighting matcher.
 *
 *
 */
public class XSpanNearQuery extends SpanQuery implements Cloneable {

    public XSpanNearQuery(SpanNearQuery source) {
        this(source.getClauses(), source.getSlop(), source.isInOrder());
    }
     ...
     */
    public XSpanNearQuery(SpanQuery[] clausesIn, int slop, boolean inOrder) {
    ...
    public SpanWeight createWeight(IndexSearcher searcher, boolean needsScores) throws IOException {
        ...
        return new XSpanNearWeight(subWeights, searcher, needsScores ? getTermContexts(subWeights) : null);
    }

    public class XSpanNearWeight extends SpanWeight {

        final List<SpanWeight> subWeights;

        public XSpanNearWeight(List<SpanWeight> subWeights, IndexSearcher searcher, Map<Term, TermContext> terms) throws IOException {
            super(XSpanNearQuery.this, searcher, terms);
        ...
        public Spans getSpans(final LeafReaderContext context, Postings requiredPostings) throws IOException {
            ...
            return (!inOrder) ? new NearSpansUnordered(slop, subSpans) : new XNearSpansOrdered(slop, subSpans);
        }
    ...
    public Query rewrite(IndexReader reader) throws IOException {
        ...
        if (actuallyRewritten) {
            try {
                XSpanNearQuery rewritten = (XSpanNearQuery) clone();
    ...
    private boolean equalsTo(XSpanNearQuery other) {
